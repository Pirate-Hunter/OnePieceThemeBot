...
def get_user_list(config, key):
    with open('{}/OnePieceBot/{}'.format(os.getcwd(), config),
              'r') as json_file:
...
class Config(object):
    ...
    WORLD_GOVT = get_user_list('elevated_users.json', 'sudos')
    ##List of id's - (not usernames) for developers who will have the same perms as the owner
    DEV_USERS = get_user_list('elevated_users.json', 'devs')
    ##List of id's (not usernames) for users which are allowed to gban, but can also be banned.
    ADMIRALS = get_user_list('elevated_users.json', 'supports')
    #List of id's (not usernames) for users which WONT be banned/kicked by the bot.
    WARLORDS = get_user_list('elevated_users.json', 'WARLORDS')
    REVOLUTIONARIES = get_user_list('elevated_users.json', 'whitelists')
