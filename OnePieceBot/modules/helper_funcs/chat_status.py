...
from OnePieceBot import (DEL_CMDS, DEV_USERS, WORLD_GOVT, SUPPORT_CHAT, ADMIRALS,
                          WARLORDS, REVOLUTIONARIES, dispatcher)
from OnePieceBot.mwt import MWT
...
def is_whitelist_plus(chat: Chat,
    ...
    return any(user_id in user
               for user in [REVOLUTIONARIES, WARLORDS, ADMIRALS, WORLD_GOVT, DEV_USERS])


def is_support_plus(chat: Chat,
                    user_id: int,
                    member: ChatMember = None) -> bool:
    return user_id in ADMIRALS or user_id in WORLD_GOVT or user_id in DEV_USERS


def is_sudo_plus(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    return user_id in WORLD_GOVT or user_id in DEV_USERS


@MWT(timeout=60 * 10
    )  # Cache admin status for 10 mins to avoid extra API requests.
def is_user_admin(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    if (chat.type == 'private' or user_id in WORLD_GOVT or user_id in DEV_USERS or
...
def is_user_ban_protected(chat: Chat,
                          ...
                          member: ChatMember = None) -> bool:
    if (chat.type == 'private' or user_id in WORLD_GOVT or user_id in DEV_USERS or
            user_id in REVOLUTIONARIES or user_id in WARLORDS or
            chat.all_members_are_administrators or
            user_id in [777000, 1087968824
                       ]):  # Count telegram and Group Anonymous as admin
        return True
...
def user_can_ban(func):
    ...
    def user_is_banhammer(update: Update, context: CallbackContext, *args,
        ...
        if not (member.can_restrict_members or member.status == "creator"
               ) and not user in WORLD_GOVT and user not in [777000, 1087968824]:
            update.effective_message.reply_text(
...
from OnePieceBot.modules import connection
